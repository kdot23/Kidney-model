...
def solveLP1Compat(T, K, matches, used_incompat, compat):
    ...
    model.addConstrs((quicksum(matchVars[t,i] for i in range(K+1) if (t,i) in matchVars) <= 1 \
                      for t in range(1,T+K+1)), "Only match with one pair")
    
    model.addConstrs((quicksum(matchVars[t,i] for t in range(1,T+K+1) if (t,i) in matchVars) + \
                      quicksum(matchVars[i+T,j] for j in range(1,K+1) if (i+T,j) in matchVars) <= 1 \
                      for i in range(1,K+1)), "undirected graph")
...
for fn in args.inputFiles:    
    ...
    for i in range(1,num_compat+1):
        ...
        max_index = solveLP1Compat(T,K,matches,used_incompat,i)[1]
