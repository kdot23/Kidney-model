        ...
        if round(x[v].value()) != 0:
            ...
            if (v[0] <= T):
                ...
                if (v[1] == 0):
                    ...
                    + "C" + "\t" + str(directed_matches[v[0],0]) + "\t" + "C" + "\t" + str(demo[v[0]-1][20]) + "\t" + str(demo[v[0]-1][20]) + "\n"
                #compatible and incompatible
                else:
                    used_incompat.add(v[1])
                    agentInfo += "C" + str(v[0]) + "\t" + str(0) + "\t" + str(directed_matches[v[1]+T,v[0]]) + "\t" \
                    + "I" + "\t" + str(directed_matches[v[0],v[1]+T]) + "\t" + "I" + "\t" + str(demo[v[0]-1][20]) + "\t" + str(demo[v[0]-1][20]) + "\n"
                    agentInfo += "I" + str(v[1]) + "\t" + str(0) + "\t" + str(directed_matches[v[0],v[1]+T]) + "\t" \
                    + "C" + "\t" + str(directed_matches[v[1]+T,v[0]]) + "\t" + "C" + "\t" + str(demo[v[1]+C-1][20]) + "\t" + str(departure_times[v[1]-1]) + "\n"
            #incompatible and incompatible
            else:
                used_incompat.add(v[0]-T)
                used_incompat.add(v[1])
                agentInfo += "I" + str(v[0]-T) + "\t" + str(0) + "\t" + str(directed_matches[v[1]+T,v[0]]) + "\t" \
                + "I" + "\t" + str(directed_matches[v[0],v[1]+T]) + "\t" + "I" + "\t" + str(demo[v[0]-1][20]) + "\t" + str(departure_times[v[0]-T-1]) + "\n"
                agentInfo += "I" + str(v[1]) + "\t" + str(0) + "\t" + str(directed_matches[v[0],v[1]+T]) + "\t" \
                + "I" + "\t" + str(directed_matches[v[1]+T,v[0]]) + "\t" + "I" + "\t" + str(demo[v[1]+C-1][20]) + "\t" + str(departure_times[v[1]-1]) + "\n"                
