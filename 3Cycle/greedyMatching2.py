...
def solveLP1Compat(T, K, matches, used_incompat, compat):
    model = Model("match compat")
    matchVars = {}
    for m in matches:
        if m[0] <= T and m[0] != compat:
            continue
        if m[0] > T and (m[0]-T) in used_incompat:
            continue
        if m[1] in used_incompat:
            continue
        if m[2] in used_incompat:
            continue
        matchVars[m] = model.addVar(vtype=GRB.BINARY, lb = 0, ub = 1, name="match"+str(m))

    
    model.addConstrs((quicksum(matchVars[t,i,j] for i in range(K+1) for j in range(K+1) if (t,i,j) in matchVars) <= 1 \
                      for t in range(1,T+K+1)), "Only match with one pair")
    
    model.addConstrs((quicksum(matchVars[t,i,j] for t in range(1,T+K+1) for j in range(K+1) if (t,i,j) in matchVars) \
                     + quicksum(matchVars[t,j,i] for t in range(1,T+K+1) for j in range(1, K+1) if (t,j,i) in matchVars)\
                     + quicksum(matchVars[i+T,j,jp] for j in range(1,K+1) for jp in range(K+1) if (i+T,j,jp) in matchVars)\
                     <= 1 for i in range(1,K+1)), "undirected graph")
    ...
    for m in matchVars:
        if m[0] == compat and round(matchVars[m].X) > 0:
            return m
results = ''
...
for fn in args.inputFiles:
    ...
    for i in range(1,num_compat+1):
        ...
        max_index = solveLP1Compat(T,K, matches, used_incompat, i)
